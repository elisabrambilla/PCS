...
int main() 
{
    ...
    for (int i=0; i<vi.size(); i++) { // per ogni dimensione elemento di vi
        ...
        for (int j=0; j<100; j++) { // creo 100 vettori di dimensione dim
            ...
            quick_sort(vec,0,vec.size()-1); // riordino il vettore j-esimo
    ...
    if (sorted) {
        std::cout<<"Il test ha avuto successo"<<std::endl;
        ...
        std::cout<<"Il test NON ha avuto successo"<<std::endl;
        return EXIT_FAILURE;}
}